...
public class LinkedListWithTail<T> implements List<T> {
	...
	public void addLast(T thing) {
		if (head == null) { // empty list, slide 176
			...
			head = p;
			tail = p; // new
			this.listSize = this.listSize + 1; // new
			ticker.tick(5);  // for the 5 statements above // new
		}
		else {
			...
			//p.next = q;
			ticker.tick(5);  // for the 5 statements not in the loop // new
		}
